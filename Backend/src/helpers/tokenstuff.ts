...
// provide key in production!! (|| not ?? so an empty-string env var still falls back)
...
export const makeGuestToken = (sub: string, nickname: string) => {
  return jwt.sign({ user_metadata: { nickname }, sub }, jwtpriv, {
    expiresIn: "30d",
  });
};
...
export const updateTokenAndAuth = (
  token: string,
  player: Player,
  name: string
) => {
